...
def get_source_politely(url, update_flag = False):
    ...
    scraperwiki.sqlite.execute("CREATE TABLE IF NOT EXISTS sources (url TEXT PRIMARY KEY, DATETIME, source_blob BLOB)")
    result = scraperwiki.sqlite.execute("SELECT url, source_blob FROM sources WHERE url = '" + url + "'")
    if len(result["data"]) == 0 or update_flag == True:
        ...
        scraperwiki.sqlite.save(unique_keys=["url"], data={"url":url, "source_blob":source}, table_name="sources")
        return source
    else:
